...
import { fetchVcCredentialsEmail, fetchVcCredentialsId } from "../services/vc-authService.js";

export const vcLoginController = async (req, res, next) => {
  try {
    ...
    const investorData = await fetchVcCredentialsEmail(data);
  

    if (!investorData) {
      return next(new AuthenticationError("vc credentials not found"));
    }
   console.log(data.password,investorData)
    ...
    const token = generateToken(investorData._id, "vc");
